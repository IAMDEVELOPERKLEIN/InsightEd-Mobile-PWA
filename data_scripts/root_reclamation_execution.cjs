...
console.log('🚀 Executing Portal Root Reclamation: Redirecting to /insighted/');
...
conn.on('ready', () => {
    ...
    const cmd = `
        ...
        sudo cp /etc/nginx/sites-available/stride.conf /tmp/stride_before_root_reclamation.bak

        echo "\\n📂 2. Rebuilding stride.conf with Root Redirect and conditional Health Probe..."
        cat <<EOF > /tmp/stride_root_reclamation.conf
server {
    listen 80;
...
server {
    ...
    # Exact match for root: Redirect to the main portal, unless User-Agent is Azure Health Probe
    location = / {
        # Azure App Gateway and Traffic Manager probes
        if (\\$http_user_agent ~* "HealthProbe|Azure|LoadBalancer|TrafficManager") {
            return 200 'healthy';
        }
        return 301 /insighted/;
    }

    # Catch-all for undefined subpaths at the root level
    location / {
        return 301 /insighted/;
    }
...
EOF
        ...
        sudo mv /tmp/stride_root_reclamation.conf /etc/nginx/sites-available/stride.conf
        ...
        echo "\\n✅ PORTAL ROOT RECLAMATION COMPLETE."
    `.trim();
