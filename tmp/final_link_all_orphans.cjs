...
async function fix() {
  try {
    ...
    // Update ALL projects where engineer_id is NOT the correct one
    const updateRes = await pool.query(
      "UPDATE engineer_form SET engineer_id = $1, engineer_name = 'DepEd Engineer' WHERE engineer_id IS DISTINCT FROM $1",
      [engineerUid]
    );
