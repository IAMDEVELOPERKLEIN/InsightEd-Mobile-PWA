    ...
    const handleDivisionChange = async (division) => {
        ...
        if (effectiveRole === 'Regional Office' || (effectiveRole === 'Central Office' && division)) {
            ...
            try {
                ...
                // Use API data directly ΓÇö schools_IERN is now the authoritative source
                                                        ...
                                                        {regionalStats.map((reg, idx) => (
                                                            <tr key={idx} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-colors border-b border-slate-50 dark:border-slate-800 group">
                                                                ...
                                                                <td className="p-5 text-center font-mono text-slate-500 text-[11px]">
                                                                    Γé▒{parseInt(reg.total_allocation || 0).toLocaleString()}
                                                                </td>
                                                                <td className="p-5 text-center font-mono text-slate-500 text-[11px]">
                                                                    Γé▒{parseInt(reg.total_contract_amount || 0).toLocaleString()}
                                                                </td>
                                                ...
                                                <div>
                                                    ...
                                                    {p.projectAllocation && (
                                                        <p className="text-[10px] font-mono text-slate-400 mt-1">
                                                            Alloc: Γé▒{Number(p.projectAllocation).toLocaleString()}
                                                        </p>
                                                    )}
                                                </div>
                                                <div className="text-right">
                                                ...
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <div className="text-right space-y-1">
                                                        <div className="bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 rounded-xl border border-emerald-100 dark:border-emerald-800">
                                                            ...
                                                            <p className="text-sm font-black text-emerald-700 dark:text-emerald-300 antialiased tracking-tight">Γé▒{Number(engStats?.total_allocation || 0).toLocaleString()}</p>
                                                        </div>
                                                        <div className="bg-white/50 dark:bg-slate-800/50 px-3 py-1.5 rounded-xl border border-emerald-100 dark:border-emerald-800">
                                                            <p className="text-[8px] font-black text-slate-400 uppercase leading-none">Total Contract</p>
                                                            <p className="text-sm font-black text-blue-700 dark:text-blue-300 antialiased tracking-tight">Γé▒{Number(engStats?.total_contract_amount || 0).toLocaleString()}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                            ...
                                                            >
                                                                ...
                                                                {effectiveRole !== 'Regional Office' && effectiveRole !== 'School Division Office' && (
                                                                    <p className="text-[10px] font-bold text-slate-400 uppercase">
                                                                        <span className="text-emerald-500">{validatedCount} Validated</span> ΓÇó <span className="text-rose-500">{forValidationCount} For Validation</span>
                                                                    </p>
                                                                )}
                                                            </div>
                                                        );
                                                                                ...
                                                                                return (
                                                                                    <div className="flex items-center gap-2">
                                                                                        <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase shrink-0 flex items-center gap-1 ${colorClass}`}>
                                                                                            ...
                                                                                            <span className="opacity-75">Score: {score}</span> ΓÇó {label}
                                                                                        </span>
                                                                                    </div>
                                                                                );
                                                                            })()}
                                            ...
                                            <div className="flex justify-between items-center mb-8">
                                                <div>
                                                    ...
                                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">
                                                        {insightsGradeLevel === 'total' ? 'All Grade Levels' : `Grade Level: ${insightsGradeLevel.toUpperCase()}`} ΓÇó Summarized by {coDistrict ? 'School' : ((coDivision || effectiveRole === 'School Division Office') ? 'District' : 'Division')}
                                                    </p>
                                                </div>
                            ...
                            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-1">
                                        School List ΓÇó {filtered.length} Schools
                                    </p>
                                </div>
                                <button
                                    onClick={() => {
